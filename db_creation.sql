...
CREATE TABLE IF NOT EXISTS `time_tracking`.`employee` (
  ...
  `account_role` ENUM('admin', 'employee') NOT NULL DEFAULT 'employee',
...
CREATE TABLE IF NOT EXISTS `time_tracking`.`task` (
  ...
  `task_status` ENUM('assigned', 'finished', 'cancelled') NOT NULL DEFAULT 'not_assigned',
  `task_deadline` DATE NOT NULL,
  `task_spent_time` INT NOT NULL DEFAULT 0,
  `task_approval_state` ENUM('approved', 'not_approved', 'new_request') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `time_tracking`.`task_archive` (
  ...
  `employee_id` INT NOT NULL,
  `task_name` VARCHAR(45) NOT NULL,
  `task_status` ENUM('approved', 'not_approved', 'new_request') NOT NULL DEFAULT 'not_assigned',
  `task_deadline` DATE NOT NULL,
  `task_spent_time` INT NOT NULL DEFAULT 0,
  `task_approval_state` ENUM('new', 'yes', 'no') NOT NULL,
  PRIMARY KEY (`project_id`, `employee_id`),
  INDEX `fk_task_employee_idx` (`employee_id` ASC),
  INDEX `fk_task_project_idx` (`project_id` ASC),
  UNIQUE INDEX `task_id_UNIQUE` (`task_id` ASC),
